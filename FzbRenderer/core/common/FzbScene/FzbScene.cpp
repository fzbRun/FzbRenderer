...
void FzbMainScene::initScene(bool compress, bool isMainScene) {
	...
	for (int i = 0; i < this->sceneMeshSet.size(); ++i) {
		...
		for (int j = 0; j < meshes_hasData.size(); ++j) {
			...
			//这里得分material是从sceneXML中得到的还是从mesh的mtl中得到的，如果是前者，则material的vertexFormat等都是正确的；否则material的vertexFormat等都有问题
	...
	for (int i = 0; i < this->sceneMeshSet.size(); ++i) {
		...
		for (auto& pair : sceneMaterials) {
			...
			else ++mesh.materialIndex;		//这个主要是有时候需要在buffer中获取相应的material
		}
	}
