...
VkDescriptorPool fzbCreateDescriptorPool(std::map<VkDescriptorType, uint32_t> bufferTypeAndNum) {
	...
	if (vkCreateDescriptorPool(FzbRenderer::globalData.logicalDevice, &poolInfo, nullptr, &descriptorPool) != VK_SUCCESS) {
...
VkDescriptorSetLayout fzbCreateDescriptLayout(uint32_t descriptorNum, std::vector<VkDescriptorType> descriptorTypes, std::vector<VkShaderStageFlags> descriptorStagesFlags, std::vector<uint32_t> descriptorCounts, std::vector<bool> bindless) {
	...
	if (vkCreateDescriptorSetLayout(FzbRenderer::globalData.logicalDevice, &layoutInfo, nullptr, &descriptorSetLayout) != VK_SUCCESS) {
...
VkDescriptorSet fzbCreateDescriptorSet(VkDescriptorPool descriptorPool, VkDescriptorSetLayout& descriptorSetLayout) {
	...
	if (vkAllocateDescriptorSets(FzbRenderer::globalData.logicalDevice, &allocInfo, &descriptorSet) != VK_SUCCESS) {
