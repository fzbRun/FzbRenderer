...
FzbShader::FzbShader(VkDevice logicalDevice, std::string path, FzbShaderExtensionsSetting extensionsSetting, bool useStaticCompile) {
	this->logicalDevice = logicalDevice;
	this->path = path;
	...
	if (pugi::xml_node pipelineInfoNode = shaderInfos.child("Pipeline")) {
		...
		if (pugi::xml_node dynamicStateInfoNode = pipelineInfoNode.child("dynamicStateInfo")) {	//目前动态只关于视口，等于sceneXML中给出的分辨率
			...
			if (pugi::xml_node resolutionNode = viewportStateInfoNode.child("resolution")) {
				pipelineCreateInfo.resolution.width = std::stof(resolutionNode.attribute("width").value());
				pipelineCreateInfo.resolution.height = std::stof(resolutionNode.attribute("height").value());
			}
			else {
				pipelineCreateInfo.resolution = { 512, 512 };
			} 
...
VkExtent2D FzbShader::getResolution() {
	return this->pipelineCreateInfo.resolution;
}
