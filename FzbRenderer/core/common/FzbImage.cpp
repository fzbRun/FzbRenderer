...
void FzbImage::fzbCreateImage(VkPhysicalDevice physicalDevice, VkDevice logicalDevice, VkCommandPool commandPool, VkQueue graphicsQueue) {
	...
	if (this->texturePath) {
		...
		FzbBuffer stagingBuffer(physicalDevice, logicalDevice, imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
...
void FzbImage::createImage() {
	...
	allocInfo.memoryTypeIndex = findMemoryType(physicalDevice, memRequirements.memoryTypeBits, this->properties);
	...
	if (UseExternal) {
		...
		GetMemoryWin32HandleKHR(logicalDevice, &handleInfo, &this->handle);
	}
...
void FzbImage::generateMipmaps() {
	...
	vkGetPhysicalDeviceFormatProperties(physicalDevice, this->format, &formatProperties);
...
void  FzbImage::createImageSampler() {
	...
	samplerInfo.addressModeU = this->addressMode;	//如果设为repeat，阴影Map边界会出现问题
	...
	if (vkCreateSampler(logicalDevice, &samplerInfo, nullptr, &this->textureSampler) != VK_SUCCESS) {
...
VkFormat fzbFindDepthFormat(VkPhysicalDevice physicalDevice) {
	return fzbFindSupportedFormat(physicalDevice, { VK_FORMAT_D32_SFLOAT, VK_FORMAT_D32_SFLOAT_S8_UINT, VK_FORMAT_D24_UNORM_S8_UINT },
