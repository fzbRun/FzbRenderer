...
__global__ void getSVONodesWeight_device(
	...
	if (hasData) {
		if (targetNodeData.indivisible == 0) {		//fatherNode一定是可分node
			...
			float distanceX = nodeData.AABB_G.rightX - nodeData.AABB_G.leftX;
			float distanceY = nodeData.AABB_G.rightY - nodeData.AABB_G.leftY;
			float distanceZ = nodeData.AABB_G.rightZ - nodeData.AABB_G.leftZ;
			...
			for (int i = 0; i < testCount; ++i) {
				...
				ray.startPos = glm::vec3(nodeData.AABB_G.leftX, nodeData.AABB_G.leftY, nodeData.AABB_G.leftZ);
				...
				if (ray.depth >= r) {	//没打中三角形也算
					...
					weight += max(dot(normalizeNormal_E, -ray.direction), 0.1f); // * max(dot(normalizeNormal_G, ray.direction), 0.1f);
				} 
