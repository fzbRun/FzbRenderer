...
struct FzbRoughconductorMaterial {
	...
	uint32_t bsdfPara = 1;
	inline static uint32_t textureNum = 3;

	static void getSceneXMLInfo(FzbMaterial* material, pugi::xml_node& roughconductorNode) {
		if (pugi::xml_node bsdfNode = roughconductorNode.child("bsdf")) {
			...
			pugi::xml_node etaNode = bsdfNode.select_node(".//rgb[@name='eta']").node();
			pugi::xml_node kNode = bsdfNode.select_node(".//rgb[@name='k']").node();
			if (etaNode && kNode) {
			...
			if (pugi::xml_node roughnessNode = bsdfNode.select_node(".//float[@name='alpha']").node()) {
	...
	static int getAttributeIndex(std::string attributeType) {
		...
		else if (attributeType == "bsdfPara") return 1;
	}
};
